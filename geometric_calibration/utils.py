...
def drag_and_drop_bbs(curr_proj, bbs_proj, grayscale_range):
    ...
    # Drag&Drop
    pts = []
    for x, y in zip(bbs_proj[:, 0], bbs_proj[:, 1]):
...
"""
def correct_bbs_coord(curr_proj, bbs_proj, grayscale_range):
    # Overlay reference bbs with projection
    fig = plt.figure(num="Drag&Drop")
    ...
    # Drag&Drop
    r2d_corrected = drag_and_drop(bbs_proj[:, 0], bbs_proj[:, 1], ax)

    return r2d_corrected
"""

"""#correct_bbs_coord
    # Input: ref image, ref_points
    # Output: r2d_corrected
"""


"""
def drag_and_drop_old(points_x, points_y, curr_plot):
    pts = []
    for x, y in zip(points_x, points_y):
        point = patches.Circle((x, y), fc="r")
        pts.append(point)
        curr_plot.add_patch(point)

    draggable_bbs = DraggablePoints(pts, curr_plot)

    return draggable_bbs.final_coord
"""
...
def search_bbs_centroids(img, ref_2d, search_area, dim_img, grayscale_range):
    ...
    and identify the bbs as black pixels inside these areas (brandis are used as probes)"""
    bbs_centroid = []
...
def angle2rotm(rot_x, rot_y, rot_z):
    """generate the rototranslator (only rotation) starting from euler angles
...
"""function r2d = project_camera_matrix(r3d,om,image_center,sid,sad,pixel_size,isocenter)
    # project 3D data starting from projection matrix based on intrinsic and
...
"""function [T,extrinsic,intrinsic] = create_camera_matrix(om,sid,sad,pixel_size,isocenter)
    # generate projection matrix starting from extrinsic and intrinsic
    # parameters ( according with the rules of creation of a projection matrix)
"""


def get_grayscale_range(img):
    """Original image is too bright, new grayscale range between min of image
    and one order of magnitude less than original image. Worst case scenario
    [0, 6553.5] (since im is loaded as uint16)"""
